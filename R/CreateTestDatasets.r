...
n_points <- 10000
start_dates <- as.Date("2022-01-01") + runif(n_points, 0, 365*3)
months_to_add <- sample(12:12, n_points, replace = TRUE)
start_lt <- as.POSIXlt(start_dates)
start_lt$mon <- start_lt$mon + months_to_add
end_dates <- as.Date(start_lt)

# Example insurance dataset with start and end dates
insurance_data <- data.frame(
  id = 1:n_points,
  latitude = runif(n_points, min = lat_min, max = lat_max),
  longitude = runif(n_points, min = lon_min, max = lon_max),
  ...
  occupancy = sample(c("Residential", "Office", "Retail", "Warehouse", "Factory"), n_points, replace = TRUE),
  construction_year = sample(1950:2023, n_points, replace = TRUE),
  total_insurable_value = round(runif(n_points, 400000, 1000000), 0),
  earthquake_AAL = round(runif(n_points, 100, 50000), 2),
  premium_rate = round(runif(n_points, 0.001, 0.01), 4),
  deductible_type = sample(c("Absolute", "Percentage"), n_points, replace = TRUE),
  deductible = NA_real_
)
...
######################
# 2.0 Create Damage Variables
# Linear example curve (simple for testing)
curve_example <- function(intensity) {
  pmin(intensity / max(intensity), 1)  # caps at 1
}

# Or a more realistic non-linear vulnerability curve
curve_example2 <- function(intensity) {
  0.05 + 0.9 * (1 - exp(-0.3 * intensity))
}
